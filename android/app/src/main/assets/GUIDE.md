...
电脑不用开浏览器。解压「触控精灵电脑端」，双击 `触控精灵.bat`，填手机上的房间码。
...
| `GetColorDep()` | 色彩位数，这里固定 32 |
| `GetDeviceID()` | ANDROID_ID |
| `Play("a.mp3")` | 播应用目录里的音频 |
| `KillApp("pkg")` | 杀后台，强杀其它 App 仍需 Root |
| `PutAttachment("a.png")` | 释放附件到应用目录 |
| `DelayS(1)` | 延时秒，图里「延时 1 秒」对应这个；`Delay` 仍是毫秒 |
| `Touch(x, y, 1000)` | 按住指定毫秒 |
| `SnapShot("cap.png")` | PNG 截屏，可带文件名 |
| `FindPic(x1,y1,x2,y2,"a.png")` | 找图，**尚未真正比对**，现在返回 -1 |
| `TickCount()` | 开机毫秒 |
| `KeepAwake(true)` / `SetScreenAlwaysOn(true)` | 免 Root 屏幕常亮，加载脚本后不锁屏。**不是** `KeepScreen`（那是冻图找色） |
| `Vibrate(ms)` | 震动 |
